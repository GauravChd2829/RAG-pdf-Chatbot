...
doc_collection = db[COLLECTION_NAME[selected_db_key]]  
    ...
    try:
        with pdfplumber.open(file_path) as pdf:
            for page in pdf.pages:
                ...
                if page_text:
                    extracted_text.append(page_text.strip())  # Strip unnecessary spaces
...
def index_document(file_name, text):
    ...
    doc_collection.insert_one({"file_name": file_name, "file_hash": file_hash, "text_content": text})

    # Adjust chunking parameters for better search results
    ...
    vectorstore.save_local(f"{FAISS_INDEX_PATH}_{selected_db_key}")  # Save separate FAISS index per category
...
def get_chat_collection():
    ...
    selected_db_key = st.session_state.get("selected_db", "iot")  # Ensure updated value
    return db[CHAT_COLLECTION_NAME[selected_db_key]]
...
def main():
    ...
    if chat_history:
        for chat in chat_history:
            st.markdown(f"<div class='question'>🦰 {chat['user']}</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='ai-response'><strong>🤖 AI Response:</strong> {chat['ai_response']}</div>", unsafe_allow_html=True)
            st.markdown(f"<sub><div class='time'>🕒 Response Time: {chat.get('response_time_seconds', '?')} seconds</sub></div>", unsafe_allow_html=True)

            if 'response_time' in chat:
                st.markdown(f"<sub>🕒 Response Time: {chat.get('response_time_seconds', '?')} seconds</sub>", unsafe_allow_html=True)
    else:
        st.info("No chat history found.")

    # User Input for AI Chat
    ...
    if user_input:
        with st.spinner("🔍 Searching for answer..."):
            ...
            st.markdown(f"<sub><div class='time'>🕒 Response Time: {response_time} seconds</sub></div>", unsafe_allow_html=True)
            st.markdown("\n")      
