...
# Fetch all feedback from MongoDB (most recent first)
feedbacks = list(feedback_collection.find({}, {"_id": 0, "name": 1, "feedback": 1}).sort("_id", -1))

if not feedbacks:
    st.info("ℹ No feedback submitted yet.")
else:
    # Convert to DataFrame
    feedback_df = pd.DataFrame(feedbacks)

    st.markdown("<h2 style='color:#4CAF50;'>User Feedback</h2>", unsafe_allow_html=True)
    for index, row in feedback_df.iterrows():
         st.markdown(f"""
        <div class="ai-response">
            <p><strong>Name:</strong> {row.get('name', 'Anonymous')}</p>
